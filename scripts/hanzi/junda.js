...
export async function populate(filename = absPath('out/entry/junda.db')) {
          ...
          stmt.run({
            data: JSON.stringify(
              sEntry.ensure({
                ...
                tag: ['junda'],
                entry: [p.character],
                reading: (p.pinyin || makePinyin(p.character))
                  .split('/')
                  .filter((s) => s),
                translation: p.english.split('/').filter((s) => s),
